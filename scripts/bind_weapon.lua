...
function weapon_binder:update(delta)
	...
	local obj=self.object local sec,st=obj:section(),ekidona_mags.WaMArray[obj:id()]
	if(IsWeapon(obj))and not(IsKnife(obj))then WeaponGrenadeAmmoDB[obj:id()]=(ekidona_mags.GetWeaponGrenadeLauncher(obj) and obj:get_ammo_in_magazine()>0)
		if(obj:weapon_grenadelauncher_status()>0)and not(ekidona_mags.GetWeaponGrenadeLauncher(obj))then WeaponMainAmmoDB[obj:id()]={obj:get_ammo_type(),obj:get_ammo_in_magazine()}end
	end local function GetAmmoMass() local adata=WeaponMainAmmoDB[obj:id()]
	...
	if(self.first_call)then
		if(ekidona_mags.isMagazine(sec)or ekidona_mags.isMSuit(sec))and(obj:parent())and(IsTrader(obj:parent()))then--IsTrader is not prepared for working
			if(ekidona_mags.isMagazine(sec))then ekidona_mags.WaMArray[obj:id()]={nil,0} else ekidona_mags.WaMArray[obj:id()]={}end
	...
	elseif(ekidona_mags.isMWeapon(sec))then
		if(st==nil)then ekidona_mags.WaMArray[obj:id()]=false end if not(ekidona_mags.WaMArray[obj:id()])and(obj:get_ammo_in_magazine()>0)then
			ekidona_mags.WaMArray[obj:id()]=ekidona_mags.GetMagName(sec,1,1,obj:get_ammo_in_magazine(),ekidona_mags.SelectAmmoTypeName(obj,obj:get_ammo_type()))
			if(ekidona_mags.WaMArray[obj:id()]==nil)then SetWeaponToMag(obj)end
		end st=ekidona_mags.WaMArray[obj:id()] local amass=system_ini():r_float_ex(sec,"inv_weight")
	...
	elseif(ekidona_mags.isMSuit(sec))then if not(st)then return end local udata,tremove,addsize=ekidona_mags.GetMagazinesOnUnload(obj),{},0
		...
		for k,v in pairs(ekidona_mags.WaMArray[obj:id()])do local magsec=ekidona_mags.GetMagFromInd(v[1])
		...
		end end for i=1,#tremove do table.remove(ekidona_mags.WaMArray[obj:id()],tremove[i]-(i-1))end obj:set_weight(amass)
...
function weapon_binder:net_spawn(se_abstract)
	...
	WeaponGrenadeAmmoDB[id]=(WeaponGrenadeAmmoDB[id] or(mdata and mdata.WeaponGrenadeAmmoDB and mdata.WeaponGrenadeAmmoDB[id])or false)
	WeaponMainAmmoDB[id]=(WeaponMainAmmoDB[id] or(mdata and mdata.WeaponMainAmmoDB and mdata.WeaponMainAmmoDB[id])or false)
	if(ekidona_mags.WaMArray[id]==nil)then
		if(ekidona_mags.isMagazine(section))then ekidona_mags.WaMArray[id]={nil,0}
		elseif(ekidona_mags.isMWeapon(section))then local ammo_have=self.object:get_ammo_in_magazine()
			if(ammo_have>0)or(math.random()>=0.75)then ekidona_mags.WaMArray[id]=ekidona_mags.GetMagName(section,1,1,ammo_have,ekidona_mags.SelectAmmoTypeName(self.object,self.object:get_ammo_type()))
				if(ekidona_mags.WaMArray[id]==nil)then SetWeaponToMag(self.object)end
			else ekidona_mags.WaMArray[id]=false end
		elseif(ekidona_mags.isMSuit(section))then ekidona_mags.WaMArray[id]={}end
...
function SetWeaponToMag(wpn) local ammo_name,id=ekidona_mags.SelectAmmoTypeName(wpn,wpn:get_ammo_type()),wpn:id()
	ekidona_mags.WaMArray[id]=ekidona_mags.GetMagName(wpn:section(),1,1,nil,ammo_name) local mags=ekidona_mags.GetMagFromInd(ekidona_mags.WaMArray[id])
