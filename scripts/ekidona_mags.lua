...
--26.02.23 : 13:19 (JST)
-- Registration
WaMArray,flag_weapon_jammed={},{}
local UIMenuActive,precahced_wmdata,precahced_magstowpn,precached_nonmagst=false,{},{},{}--meh
...
function save_state(mdata)
	mdata.WaMArray=WaMArray
	mdata.flag_weapon_jammed=flag_weapon_jammed
end
function load_state(mdata)
	WaMArray=mdata.WaMArray
	flag_weapon_jammed=mdata.flag_weapon_jammed
end
...
function GetMagazinesOnUnload(itm) if not(itm and isMSuit(itm:section()))then return end local magcnt=0 -- local outfit=(itm or db.actor:item_in_slot(7) or db.actor:item_in_slot(15))
	for k,v in pairs(WaMArray[itm:id()] or {})do magcnt=(magcnt+precahced_wmdata[precahced_wmdata[v[1]]][4])end
	return({magcnt,GetSuitUnloadCond(itm)})
end
function IsAppropriateMagazine(wpn,mag) return(precahced_wmdata[wpn][2][mag])end
function weapon_jammed() flag_weapon_jammed[db.actor:active_item():id()]=true end
...
function unload_mag_functor(item) if(UIMenuActive)then return end
	if(isMagazine(item:section()))and(WaMArray[item:id()][2]>0)then return game.translate_string("st_eject_ammo")
	elseif(isMWeapon(item:section())and not(GetWeaponGrenadeLauncher(item))and((WaMArray[item:id()])or(item:get_ammo_in_magazine()>0)))then return game.translate_string("st_eject_mag")end
	return
end
...
function outfit_unloading_menu(item) if(UIMenuActive)then return end
	if(#WaMArray[item:id()]>0)then return game.translate_string("st_unload_outfit")end return
end
...
function CreateMagazine(sec,pos,lvid,gvid,aid,ammo,count,rkoeff) ammo=(ammo and GetRealAmmo(ammo))
	...
	if(ammo and count and count>0)then WaMArray[mag.id]={GetAmmoIndFromMag(sec,ammo),count}
	else WaMArray[mag.id]={nil,0}end
end
...
function WeaponEjectMag(weapon) local wmag=WaMArray[weapon:id()]
	...
	WaMArray[new_ammo_mag.id]={precahced_wmdata[magsec][2][ammosec],weapon:get_ammo_in_magazine()} weapon:set_ammo_elapsed(0) WaMArray[weapon:id()]=false
end
function MagEjectAmmo(mag,cnt) local st=WaMArray[mag:id()] if not(cnt)or(cnt>st[2])then cnt=st[2]end
...
function on_key_release(key) if(UIMenuActive)then return end local weapon=db.actor:active_item()
	...
	if not(weapon and IsWeapon(weapon) and isMWeapon(weapon:section()) and not(GetWeaponGrenadeLauncher(weapon)))or(flag_weapon_jammed[weapon:id()])or(weapon:get_state()==7)then return end
	local unloads={db.actor:item_in_slot(7),db.actor:item_in_slot(15)}
	for k,v in pairs(unloads)do if(v and isMSuit(v:section()) and #WaMArray[v:id()]>0)then local switcher=unloads[math.abs(k-3)]
		mag_trans_wpn_ui(v,weapon,nil,(switcher and isMSuit(switcher:section()))and switcher):ShowDialog(true) return
	end end
	--local outfit=db.actor:item_in_slot(7) if not(outfit and precahced_wmdata[outfit:section()] and #WaMArray[outfit:id()]>0)then return end mag_trans_wpn_ui(outfit,weapon):ShowDialog(true)
...
function animation_end(item,section,motion,state,slot)-- printf("%s: %s; %s",section,state,slot)
	if(state==7)and(flag_weapon_jammed[item:id()])then flag_weapon_jammed[item:id()]=nil end
	if(not(reloadtb[5])or(reloadtb[5]~=item:id()))then if(state~=2)then reloadtb={0,0,0,nil,nil,nil}end return end
	if(state==7)then
		SetWeaponAmmoParams(item,reloadtb[3],reloadtb[1]) WaMArray[item:id()]=(reloadtb[4] and precahced_wmdata[reloadtb[4]][0])
		if(reloadtb[6])then if(reloadtb[6][1])then table.remove(WaMArray[reloadtb[6][2]],reloadtb[2])
...
function WeaponAttemptToLoadMagazine(weapon,item,outfit) local wsec,msec=weapon:section(),item:section()
	...
	local mst=WaMArray[item:id()] local ammotype=SelectAmmoType(weapon,precahced_wmdata[msec][3][mst[1]])
...
function drag_item(item,item2,from_slot,to_slot)
	...
	or(item:id()==item2:id())then return end local st=WaMArray[item2:id()]
	...
	elseif(isMagazine(item:section()))and(isMSuit(item2:section()))then local st,pdata=WaMArray[item:id()],GetMagazinesOnUnload(item2)
		if(pdata[1]+GetMagSize(item:section())>pdata[2])or not(st[1] and st[2]>0)then return end table.insert(WaMArray[item2:id()],{precahced_wmdata[item:section()][0],st[1],st[2]})
		alife():release(alife_object(item:id()),true) xr_sound.set_sound_play(0,"inv_stack")
	end
end
-- Trading/Barter
function actor_on_trade(obj,sell_bye,money)
	local sec,st=obj:section(),WaMArray[obj:id()]
	if(sell_bye)then
		if(isMagazine(sec))and(st[1])and(st[2]>0)then
			create_ammo(precahced_wmdata[sec][3][st[1]],db.actor:position(),db.actor:level_vertex_id(),db.actor:game_vertex_id(),0,st[2]) WaMArray[obj:id()]={nil,0}
		elseif(isMWeapon(sec))and(st)then give_object_to_actor(GetMagFromInd(st)) WaMArray[obj:id()]=false local trd=level.object_by_id(inventory_upgrades.victim_id)
	...
	elseif(isMagazine(sec))and(st[1])and(st[2]>0)then local prnt=level.object_by_id(inventory_upgrades.victim_id) if not(prnt)then return end--meh
		create_ammo(precahced_wmdata[sec][3][st[1]],prnt:position(),prnt:level_vertex_id(),prnt:game_vertex_id(),prnt:id(),st[2]) WaMArray[obj:id()]={nil,0}
	end
end
-- Ammo sort
...
function GetRealDamage(shit) if(shit.type==hit.radiation or shit.power<=0)then return(false)end local protection=0 -- total meh (useless)
	for k,v in pairs({7,12})do if(db.actor:item_in_slot(v))then local itm=db.actor:item_in_slot(v)
		protection=(protection+((sini:r_float_ex(itm:section(),hit_to_section[shit.type]) or 0)*itm:condition()))
	end end db.actor:iterate_inventory(function(temp,item) if(IsArtefact(item) and(db.actor:is_on_belt(item) or(db.actor:item_in_slot(15) and db.actor:item_in_slot(15):id()==item:id())))then
		protection=(protection+(artefact_degradation.get_artefact_adsorbation(item,hit_to_section[shit.type])*item:condition()))
	end end) return(shit.power-protection>=0.1)
end
...
--function ammo_sort_ui:actor_on_before_hit(shit) if(self and GetRealDamage(shit))then self:ExitMenu()end end
...
function ammo_trans_mag_ui:InitControls() self:SetWndRect(Frect():set(0,0,1024,768)) self:SetWndPos(vector2():set(0,0))
	...
	end for k,v in pairs({{magform,WaMArray[self.mag:id()][2]},{ammoform,self.ammo:ammo_get_count()}})do local pos=v[1]:GetWndPos()
...
function ammo_trans_mag_ui:OnButton_out()
	local ammomax,smag=sini:r_float_ex(self.ammo:section(),"box_size"),WaMArray[self.mag:id()]
...
function ammo_trans_mag_ui:OnButton_in()
	...
	local magmax,smag=GetMagAmmoSize(magsec,GetAmmoIndFromMag(magsec,self.ammo:section())),WaMArray[self.mag:id()]
...
--function ammo_trans_mag_ui:actor_on_before_hit(shit) if(self and GetRealDamage(shit))then self:CloseMenu()end end
...
function mag_trans_wpn_ui:GenerateMagsUI() local ly=0--self.list:RemoveAll()
	for k,v in pairs(WaMArray[self.outfit:id()])do local msec=precahced_wmdata[v[1]]
		if(self.mforms[k])then self.mforms[k]:Rename(msec,v[2],v[3],(self.onlyunload and #WaMArray[self.outfit:id()]>1 and((k==1 and 2)or(k==#WaMArray[self.outfit:id()] and 1)or 3)))else
		self.mforms[k]=set_mag_list_item(msec,v[2],v[3],(self.onlyunload and #WaMArray[self.outfit:id()]>1 and((k==1 and 2)or(k==#WaMArray[self.outfit:id()] and 1)or 3)))
...
function mag_trans_wpn_ui:SortThatMag(ind,sort) local st=(WaMArray[self.outfit:id()]) local nst={} if not(st[ind])then return end
	...
	WaMArray[self.outfit:id()]=nst self:GenerateMagsUI() xr_sound.set_sound_play(0,"inv_stack")
end
function mag_trans_wpn_ui:AttachThatMag(ind) local st=(WaMArray[self.outfit:id()]and WaMArray[self.outfit:id()][ind]) if not(st)then return(false)end
	if not(self.onlyunload)then
		...
		reloadtb={st[3],ind,ammotype,precahced_wmdata[st[1]],self.weapon:id(),{true,self.outfit:id()}} local pmag,ammohave=WaMArray[self.weapon:id()],self.weapon:get_ammo_in_magazine()
		if(pmag)then local uused=false
			for k,v in pairs({self.outfit,self.switchobj})do if(v)then local pdata=GetMagazinesOnUnload(v)
				if((pdata[2]-pdata[1])>=GetMagSize(GetMagFromInd(pmag)) and ammohave>0)then
					table.insert(WaMArray[v:id()],{pmag,GetAmmoIndFromMag(GetMagFromInd(pmag),SelectAmmoTypeName(self.weapon,self.weapon:get_ammo_type())),ammohave})
					WaMArray[self.weapon:id()]=false self.weapon:set_ammo_elapsed(0) uused=true break
				end
		...
		table.remove(WaMArray[self.outfit:id()],ind) self.list:RemoveWindow(self.mforms[ind]) table.remove(self.mforms,ind) self:GenerateMagsUI()
	end
end
function mag_trans_wpn_ui:OnKeyboard(dik,keyboard_action)
	...
	if(keyboard_action==ui_events.WINDOW_KEY_PRESSED)then
		...
		elseif(#WaMArray[self.outfit:id()]>1)and(self.onlyunload)then
			...
			elseif(bind==key_bindings.kBACK or dik==DIK_keys.DIK_NUMPAD2 or dik==DIK_keys.DIK_DOWN)then 
				for k,v in pairs(self.mforms)do if(v.button:IsCursorOverWindow())then if(k<#WaMArray[self.outfit:id()])then self:SortThatMag(k,2)end return(true)end end
...
--function mag_trans_wpn_ui:actor_on_before_hit(shit) if(self and GetRealDamage(shit))then self:ExitMenu()end end
