...
function inv_add_info:__init() super() local xml=CScriptXmlInit() xml:ParseFile("ui_eki_mags.xml")
	self.WPNFrame=xml:InitStatic("actor_weapon_info",self) local lpos=self.WPNFrame:GetWndPos() self.WPNPos={lpos.x,lpos.y} self.WPNSize={self.WPNFrame:GetWidth(),self.WPNFrame:GetHeight()}
...
function inv_add_info:UpdateWpnIcons(riser) local color,calpha=fcolor():set(self.WPNFrame.mag:GetTextureColor())
	...
	end change_alpha() self.WPNFrame:Show(calpha>0 and not(axr_main.weapon_is_zoomed))
end
function inv_add_info:Update() CUIScriptWnd.Update(self)
	...
	if(sec)and(IsWeapon(wpn))and not(IsKnife(wpn))then
		...
		if(ekidona_mags.GetMagazinesDB(wpn:id()))then msec=ekidona_mags.GetMagFromInd(ekidona_mags.GetMagazinesDB(wpn:id())[1])end
		...
		local size={fary[3]-fary[1],fary[4]-fary[2]} self.WPNFrame.mag:SetWndSize(vector2():set(size[1]*(self.WPNSize[1]/50),size[2]*(self.WPNSize[2]/50)))
		self.WPNFrame:SetWndPos(vector2():set(self.WPNPos[1]-((size[1]-50)*(self.WPNSize[1]/50)),self.WPNPos[2]-((size[2]-50)*(self.WPNSize[2]/50))))
		--[[self.WPNFrame.ammo:SetWndPos(vector2():set(size[1]-self.WPNSize[1],size[2]-self.WPNSize[2]))
		if(ekidona_mags.GetMagazinesDB(wpn:id()) and not(ekidona_mags.GetWeaponGrenadeLauncher(wpn)))then
			self.WPNFrame.ammo:SetText(ekidona_mags.GetMagazineAmmoCount(ekidona_mags.GetMagazinesDB(wpn:id())[2]))
		else self.WPNFrame.ammo:SetText(wpn:get_ammo_in_magazine())end]]
