...
function motivator_binder:net_spawn(se_abstract)
	...
	--[[
	local visual = system_ini():r_string_ex(npc:section(),"set_visual")
	...
	npc:set_callback(callback.use_object, self.use_callback, self)
	...
	if not npc:alive() then
		...
		if not (se_obj.m_level_vertex_id < 4294967295 and npc:accessible(se_obj.m_level_vertex_id)) then
			...
			local v_id = npc:accessible_nearest(level.vertex_position(se_obj.m_level_vertex_id), vector():set(0,0,0))
	...
	elseif (debug_cmd_list.debug_visuals and debug_cmd_list.debug_visuals[npc:id()]) then
		npc:set_visual_name(debug_cmd_list.debug_visuals[npc:id()])
		debug_cmd_list.setup_debug_logic(npc,self.st,self.loaded)
	else
...
function motivator_binder:hit_callback(obj, amount, local_direction, who, bone_index)
	--alun_utils.debug_write(strformat("START motivator_binder:hit_callback %s",obj and obj:name()))
	if (amount <= 0 or who == nil) then
		--alun_utils.debug_write(strformat("END motivator_binder:hit_callback %s",obj and obj:name()))
		return 
	end
