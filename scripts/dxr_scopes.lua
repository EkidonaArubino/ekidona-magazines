...
local function drag_scope(addon,weapon,from_slot,to_slot) local asec,wsec=addon:section(),weapon:section()
	if not(addons_table[asec] and IsWeapon(weapon) and not(ekidona_mags.GetWeaponGrenadeLauncher(weapon))
	and from_slot==EDDListType.iActorBag and(to_slot==EDDListType.iActorSlot or to_slot==EDDListType.iActorBag))then return end
...
function attach_addon(addon,weapon) local asec,wsec=addon:section(),weapon:section()
	if not(addons_table[asec] and IsWeapon(weapon) and not(ekidona_mags.GetWeaponGrenadeLauncher(weapon)))then return end
...
function detach_addon(weapon) if not(weapon and IsWeapon(weapon) and not(ekidona_mags.GetWeaponGrenadeLauncher(weapon)))then return end local wsec,addonsec=weapon:section()
	local parent_section=system_ini():r_string_ex(wsec,"parent_section") if not(parent_section and wsec~=parent_section)then return end
...
function context_functor(weapon) if not(weapon:parent() and weapon:parent():id()==0)then return end
	if not(weapon and not(ekidona_mags.GetWeaponGrenadeLauncher(weapon)))then return end local wsec,addonsec=weapon:section()
	local parent_section=system_ini():r_string_ex(wsec,"parent_section") if not(parent_section and wsec~=parent_section)then return end
	addonsec=wsec:sub(string.len(parent_section)+2,-1) if not(system_ini():section_exist(addonsec))then printf("!ERROR! addon section doesn't exitst: %s",addonsec) return end
